...
def download_zip_with_filter(url, output_dir, filter_path="", skip_depth=None):
...
def download_tag_with_filter(github_user, github_repository, tag, output_dir, filter_path="", skip_depth=None):
    tags = github_tags(github_user, github_repository)
    url = next(x["zipball_url"] for x in tags if x["name"] == tag)
    download_zip_with_filter(url, output_dir, filter_path, skip_depth)


def download_latest_tag_with_filter(github_user, github_repository, output_dir, filter_path="", skip_depth=None):
    tags = github_tags(github_user, github_repository)
    url = tags[0]["zipball_url"]
    download_zip_with_filter(url, output_dir, filter_path, skip_depth)


def download_sha_with_filter(github_user, github_repository, sha, output_dir, filter_path="", skip_depth=None):
