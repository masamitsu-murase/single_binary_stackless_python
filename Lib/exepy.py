...
def update_resource(resource_id, data, exe_dll_filename, lang_id=0,
                    data_type=RT_RCDATA):
    ...
    handle = begin_update_resource(exe_dll_filename, False)
    if handle == 0:
    ...
    result = update_resource_api(handle, data_type, resource_id, lang_id,
                                 c_buffer, len(data))
    if result == 0:
    ...
    result = end_update_resource(handle, False)
    if result == 0:
        raise RuntimeError("EndUpdateResource failure")
